...
public class NettyServerUtil {
	...
	protected static void handle(final ChannelHandlerContext ctx, final DatagramPacket msg) throws Exception{
		...
		{
			byte[] array = ArrayUtils.subarray(datagramPacket, headerEndIndex + 4,
					datagramPacket.length);
			// System.out.println(new String(array));
			for (int i = 0; i < array.length; i++) {
				if (array[i] == '\r' && array[i + 1] == '\n' && array[i + 2] == '\r'
		...
		String headerString = FreemarkerUtil.generateString(new File(NettyServerUtil.class.getResource("/200.txt").getFile()), responseHeaderMap);
