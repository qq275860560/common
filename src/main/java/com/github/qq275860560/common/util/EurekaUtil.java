...
public class EurekaUtil {
		...
		try {
			...
			MyDataCenterInstanceConfig instanceConfig = new MyDataCenterInstanceConfig() {
				...
				public String getHostName(boolean refresh) {// 显示到ureka
					return properties.getProperty("instanceId") != null ? properties.getProperty("instanceId")
							: super.getHostName(refresh);
				}

				@Override
				public String getIpAddress() {// 显示到ureka
					return properties.getProperty("ipAddr") != null ? properties.getProperty("ipAddr")
							: super.getIpAddress();
				}
	...
	public static void backInService(String appId, String instanceId, String status) {
		if (!status.equals("UP") && !status.equals("DOWN") && !status.equals("STARTING")
				&& !status.equals("OUT_OF_SERVICE") && !status.equals("UNKNOWN")) {
			log.info("不存在此状态");
			return;
		}
	...
	private static String listInstancesByVipAddress(String vipAddress) {
		...
		if (true)
			return JsonUtil.toJSONString(serverInfos);
	...
	public static void main(String[] args) throws Exception {
		start();
		log.info(JsonUtil
				.toJSONString(EurekaUtil.getInstanceIpAndPort(EurekaUtil.properties.getProperty("eureka.vipAddress"))));
		if (true)
			return;
		...
		while (true) {
			heartbeat(EurekaUtil.properties.getProperty("eureka.name"),
	...
	public static void start() throws Exception {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// 查询vipAddress域名下的所有服务
				// List<InstanceInfo> serverInfos =
				// eurekaClient.getInstancesByVipAddress(vipAddress, false);
				// log.info(vipAddress+"已注册服务="+JsonUtil.toJSONString(serverInfos));
			}
		}).start();
