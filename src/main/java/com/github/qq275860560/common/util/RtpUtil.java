...
public class RtpUtil {
	...
	public static void main(String[] args) throws Exception {
		...
		//File file = new File("D:/workspace/github-qq275860560-common/src/main/resources/rtp/1.dat");
...
class PS{
	...
	public PS(byte[] buffer, int off, int len) {
		...
		//PSSystemHeader
		if(buffer[off]==0x00 && buffer[off+1]==0x00 && buffer[off+2]==0x01 && (buffer[off+3] & 0xff)==0xBB ){
			off+=4;
		...
		if(buffer[off]==0x00 && buffer[off+1]==0x00 && buffer[off+2]==0x01 ){
			...
			if( (buffer[off] & 0xff)==0xBC ){
				...
				log.info( "视频编码格式="+String.format("%02x", buffer[off+6]) );//如果为1b,表示h264视频编码格式,{MPEG-4 视频流： 0x10；H.264 视频流： 0x1B； SVAC 视频流： 0x80；}
				log.info( "视频流="+String.format("%02x", buffer[off+7]) );//如果为e0,表示视频流,0x(C0~DF)指音频，0x(E0~EF)为视频
				log.info( "音频编码格式="+String.format("%02x", buffer[off+7+15]) );//如果为90,表示G.711音频编码格式,,{G.711 音频流： 0x90； G.722.1 音频流： 0x92 G.723.1 音频流： 0x93； G.729 音频流： 0x99； SVAC音频流： 0x9B}。
			...
			}else if((buffer[off] & 0xff)>=0xE0 &&(buffer[off] & 0xff)<=0xD0EF ||  (buffer[off] & 0xff)>=0xC0 &&(buffer[off] & 0xff)<=0xD0  ){			
...
如果要把音频Audio也打包进PS 封装，只需将数据加上PES header 放到视频PES 后就可以了。顺序如下：PS 包=PS头|PES(video)|PES(audio)；
...
class RTPPacket {
	...
	public RTPPacket(byte[] buffer, int off, int len) {
		...
		this.payloadType = new UnsignedByte(c & 0x7F); //负载类型,96 表示PS 封装，建议97 为MPEG-4，建议98 为H264,若负载类型为96，则采用PS 解复用，将音视频分开解码。若负载类型为98，直接按照H264 的解码类型解码。
		...
		if(buffer[off]==0x00 && buffer[off+1]==0x00 && buffer[off+2]==0x01 && (buffer[off+3] & 0xff)==0xBA ){
...
final class UnsignedInt extends UnsignedNumber {
    ...
    public static UnsignedInt fromBytes(byte[] c, int idx) {
        ...
        number.value = (((c[idx] << 24) & 0xFF000000L) 
        				| ((c[idx + 1] << 16) & 0xFF0000L) 
        				| (c[idx + 2] << 8 & 0xFF00L)
        				| (c[idx + 3] & 0xFFL));
        return number;
    }
